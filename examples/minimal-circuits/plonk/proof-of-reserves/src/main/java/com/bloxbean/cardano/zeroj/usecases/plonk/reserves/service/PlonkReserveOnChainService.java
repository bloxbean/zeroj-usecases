...
public final class PlonkReserveOnChainService {
    ...
    public PlonkReserveOnChainService(BackendService backendService, Account adminAccount) {
        this.backendService = backendService;
        this.adminAccount = adminAccount;
    }

    public Result verifyOnChain(PlonkReserveProofService.ProofBundle bundle) throws Exception {
        ...
        if (!lockResult.isSuccessful()) {
            throw new IllegalStateException("PlonK reserve lock tx failed: " + lockResult.getResponse());
        }
        String lockTxHash = lockResult.getValue();
        waitForTx(lockTxHash);
        System.out.println("[plonk-reserves] lockConfirmed=" + elapsedMillis(lockWaitStartNanos) + "ms");
        ...
        if (!unlockResult.isSuccessful()) {
            throw new IllegalStateException("PlonK reserve unlock tx failed: " + unlockResult.getResponse());
        }
        String unlockTxHash = unlockResult.getValue();
        waitForTx(unlockTxHash);
        System.out.println("[plonk-reserves] unlockConfirmed=" + elapsedMillis(unlockWaitStartNanos) + "ms");
        System.out.println("[plonk-reserves] onChainTotal=" + elapsedMillis(startNanos) + "ms");
    ...
    private Utxo findScriptUtxo(String scriptAddress, String txHash) throws Exception {
        ...
        throw new IllegalStateException("No PlonK reserve script UTXO found for tx " + txHash);
    }
