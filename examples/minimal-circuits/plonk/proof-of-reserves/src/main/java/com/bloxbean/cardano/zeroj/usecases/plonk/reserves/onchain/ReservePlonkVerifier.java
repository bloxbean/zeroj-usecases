package com.bloxbean.cardano.zeroj.usecases.plonk.reserves.onchain;
...
@SpendingValidator
public class ReservePlonkVerifier {
    ...
    private static boolean reservePolicy(PlutusData datum) {
        PlutusData inputs = Builtins.unListData(datum);
        PlutusData rest1 = Builtins.tailList(inputs);
        BigInteger assetValue = Builtins.asInteger(Builtins.headList(rest1));
        PlutusData rest2 = Builtins.tailList(rest1);
        BigInteger claimedLiabilities = Builtins.asInteger(Builtins.headList(rest2));
        return assetValue.compareTo(claimedLiabilities) >= 0;
    }
}
